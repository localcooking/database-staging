...
------------+------------------------+-----------+----------+---------+----------+--------------+-------------
 email      | character varying(255) |           | not null |         | extended |              | 
 auth_token | bytea                  |           | not null |         | extended |              | 
Indexes:
...
---------------+-----------------------------+-----------+----------+-----------------------------------------+----------+--------------+-------------
 ...
 session_token | bytea                       |           | not null |                                         | extended |              | 
...
CREATE TABLE IF NOT EXISTS sessions (
  ...
  session_token BYTEA NOT NULL PRIMARY KEY,
...
Referenced by:
    TABLE "carts" CONSTRAINT "carts_item_revision_id_fkey" FOREIGN KEY (item_revision_id) REFERENCES item_revisions(item_revision_id) ON DELETE RESTRICT
...
Indexes:
    "carts_user_id_item_revision_id_key" UNIQUE CONSTRAINT, btree (user_id, item_revision_id)
Check constraints:
    "carts_quantity_check" CHECK (quantity > 0)
Foreign-key constraints:
    "carts_item_revision_id_fkey" FOREIGN KEY (item_revision_id) REFERENCES item_revisions(item_revision_id) ON DELETE RESTRICT
