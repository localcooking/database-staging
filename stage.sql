...
----------------+-----------------------------+-----------+----------+----------------------------------+----------+--------------+---------------------------------------------------------------------------------------
 ...
 deactivated_on | timestamp without time zone |           |          |                                  | plain    |              | Allows a user to pseudo-delete their account without our record-books being corrupted
Indexes:
...
CREATE FUNCTION logout(uuid) RETURNS INT
AS 'DELETE FROM sessions WHERE session_token = $1 RETURNING user_id;'
  LANGUAGE SQL
  VOLATILE
  RETURNS NULL ON NULL INPUT;
...
-------------------------+-----------------------------+-----------+----------+----------------------------------+----------+--------------+-------------
 ...
 revoked_on              | timestamp without time zone |           |          |                                  | plain    |              | 
...
CREATE TABLE IF NOT EXISTS chefs (
  ...
  revoked_on TIMESTAMP, -- Pseudo-deletion
...
CREATE FUNCTION enroll(INT, VARCHAR, subscription_type, TIMESTAMP) RETURNS INT
AS 'INSERT INTO chefs (user_id, public_name, subscription, subscription_expiration) VALUES ($1, $2, $3, $4) RETURNING chef_id;'
  LANGUAGE SQL
  VOLATILE
  RETURNS NULL ON NULL INPUT;
...
CREATE FUNCTION get_chef_credentials(INT) RETURNS record AS
$$
  ...
  AS credential_id, title, description
    ...
    WHERE chef_credentials.chef_id = $1;
$$
  LANGUAGE SQL
  STABLE
  RETURNS NULL ON NULL INPUT;
...
CREATE TABLE IF NOT EXISTS items (
  ...
  deleted_on TIMESTAMP, -- pseudo-deletion; doesn't actually delete from db for order book
...
CREATE FUNCTION get_menu_items(INT) RETURNS record AS
$$
  ...
  AS item_id, deleted_on, created_on
    FROM menu_item_mapping
         INNER JOIN items
             ON menu_item_mapping.item_id = items.item_id
    WHERE menu_item_mapping.menu_id = $1
...
CREATE FUNCTION get_cart(INT) RETURNS record AS
$$
